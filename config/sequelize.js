...
// set FORCE_DB_SYNC = true in the config, or the program parameters to drop the database,
...
sequelize
    .sync({
        force: config.db.FORCE_DB_SYNC === 'true'
    })
    .then(function () {
        winston.info("Database " + (config.db.FORCE_DB_SYNC === 'true' ? "*DROPPED* and " : "") + "synchronized");
    }).catch(function (err) {
